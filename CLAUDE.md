...
- 対象教科: **社会**（日本地理・世界地理・歴史・公民。実装済み）＋ **理科**（生物・化学・物理・地学。Phase3で追加済み、計266問）。
...
**Phase5-4（生徒選択時に新学習記録GASから過去のAttempt/AnswerRecordを取得しMemoryStorageへ復元）は完了（2026-08-23実施）。** `services/learning-record-service.js`へ`fetchStudentLearningRecords(studentId)`（GET、`action=getStudentHistory`）を追加した（既存POST3関数は無変更）。新規`features/history/learning-record-restore-integration.js`の`restoreStudentLearningRecords(studentId)`が、取得した`attempts`/`answerRecords`を既存`createAttempt`/`saveAttempt`・`createAnswerRecord`/`saveAnswerRecord`経由でAttempt/AnswerRecord Repositoryへupsertする（`attemptId`／`attemptId::questionId`の既存keyベースupsertをそのまま利用、`memoryStorage.clear()`は不使用）。`app.js`は`handleHomeStudentSelect()`のみ配線：生徒選択時にまず現状のMemoryStorageでHomeを即時表示（既存`renderHomeForStudent()`と同じ同期呼び出し）し、裏で復元をfire-and-forget実行、成功時のみ「復元完了時点でも選択中studentIdが復元対象と一致している場合」に限りHomeを再描画する（生徒切替中の誤上書きを防ぐガード）。生徒選択時のみ取得し、`returnToHome()`は無変更のままGAS再取得を行わない。GAS取得失敗時はconsole.errorのみでMemoryStorageは現状維持し、生徒選択・通常学習は継続可能（「履歴取得失敗」と「履歴0件」を区別し、失敗時に0件で上書きすることはない）。`sourceType`/`testSetId`はAttemptモデルが未対応のため今回も配線せず（Phase5-6予定、GASレスポンスに含まれていても`createAttempt()`通過後は自動的に除外されることを実機確認済み）。実GAS（本番Web App URL、架空ID使用）で`completed`/`isCorrect`がJSON boolean型（`"TRUE"`/`"FALSE"`文字列ではない）で返ることを実測確認済み。headless Chromeによるブラウザテスト（Case1〜13、モックGAS）32/32 OK、Attempt/AnswerRecordモデル単体確認（Case14）・実GAS boolean型確認（Case15）もOK。A/B生徒切替競合・復元中Quiz開始競合・returnToHome非再取得・GAS失敗時継続・375px幅表示、いずれも実機確認済み。テストで本番Spreadsheetへ書き込んだ架空データ（`TEST_PHASE5_4_...`系）はテスト後にユーザーがcleanup済み（実在生徒データは未使用）。

**Phase5-5（ブラウザリロードを跨いだ永続履歴復元の本番E2E確認）は完了（2026-08-23（日）実施）。** 本Taskは実装Taskではなく、Phase5-3/5-4で実装済みの永続化経路（学習記録GASへの保存→`getStudentHistory`によるMemoryStorageへの復元）を、本番Web App／本番学習記録Spreadsheetに対して実際に確認するE2E確認Taskであり、**repositoryのコード変更は0件**（GAS側`Code.gs`/`SheetHelpers.gs`も無変更）。架空生徒で「通常学習→新GASへ保存→`page.reload()`→同一生徒再選択→`getStudentHistory`→MemoryStorage復元→Home/History/Weakness復元」の一連のE2Eが成功し、新規browser/context（ブラウザ完全終了相当）でも復元成功を確認。`returnToHome()`経由では不要な`getStudentHistory`再取得が発生しないこと、学習記録GAS取得失敗時もHome表示・通常学習が継続可能なことも実機確認済み。

調査過程で、学習記録GASへの高速連続書き込み時に低頻度のAnswerRecord欠落らしき事象（1回のみ観測）が見つかったため、追加で2種類の診断テストを実施した。(1) フロントを介さない直接HTTPテスト（本番学習記録GASへ`saveAnswerRecord`を95件送信、逐次・sleepなし高速逐次・20件並列・Attempt3回連続・段階的確認・ブラウザ類似タイミングの6パターン、TEST_P55_DIRECT_A〜F）：`ok:true`85件・`ok:false`10件（並列負荷時のLockServiceタイムアウトのみ、想定どおりの失敗処理）、`ok:true`に対する保存は85/85件で欠落0。(2) 送信questionIdとHTTPレスポンスを1件単位で記録するよう診断を強化したブラウザE2E（TEST_PHASE5_5_BROWSER_FINAL、1 Attempt・5問）：送信した`unique(attemptId::questionId)`5件と、Spreadsheet保存キー5件が完全一致（`missing`/`extra`とも空集合）、`page.reload()`後・新browser/context復元後ともにHistory総回答数5問で一致。

以前（1回のみ）観測された「5回答→AnswerRecord4件」という事象については、当時のテストログに送信questionIdとHTTPレスポンスの1対1対応が記録されておらず、真因を確定できなかった。その後の直接HTTPテスト・キー完全突合ブラウザE2Eでは欠落が再現されなかったため、**「GAS側の欠落バグだった」「テスト操作ミスだった」のいずれとも断定しない**。現行の永続化経路（学習記録GASへの保存・`getStudentHistory`による復元）については、これらの診断テストにより正常性を確認できた、という扱いとする。この経緯を踏まえ、`SpreadsheetApp.flush()`の追加や、その他のGASコード変更は行っていない。

テストで本番Spreadsheetへ書き込んだ架空データ（`TEST_PHASE5_5_STUDENT...`系・`TEST_P55_DIRECT_A`〜`F`系・`TEST_PHASE5_5_BROWSER_FINAL`系、いずれも実在生徒データではない）はcleanup済み（7 studentId全てでattempts/answerRecordsとも0件を実機確認）。

**Phase5-6（Attempt生成箇所への`sourceType`/`testSetId`配線）は完了（2026-08-23（日）実施）。** `features/history/attempt-model.js`の`createAttempt()`へ`sourceType`/`testSetId`を追加（`startedAt`/`completedAt`と同じ`input.xxx ?? null`パターン、省略時・旧データはnull＝起点不明）。`app.js`の`beginAttemptAndShowQuiz(sourceType, testSetId = null)`を起点として、`startQuiz()`（通常学習）→`sourceType="normal"`、`startPracticeSession()`（Home「苦手を復習」「復習する」）→`PRACTICE_TYPE_TO_SOURCE_TYPE`マッピング経由で`weak_review`/`dormant_review`、`startTestSetGroupQuiz()`（TestSet実行）→`sourceType="testset"`＋`features/test-set-runner/test-set-runner.js`の新規`getRunnerTestSetId()`が返す実TestSetId、をそれぞれ明示的に渡す配線とした（「値が無ければnormal」という後方互換fallbackはしない）。`features/test-set-runner/test-set-runner-state.js`のrunnerStateへ`testSetId`保持を追加し、`finishRun()`/`abortRun()`による既存の全状態リセットにそのまま乗るため、TestSet終了・中断後に`testSetId`が残留しないことを実機確認済み。複数fieldのTestSetでは各グループが別Attemptになる一方、`testSetId`は全グループで共通に維持されることも確認済み。通常Quiz内の間違い直しラウンド（`retryWrongEnabled`）は既存仕様どおり新規Attemptを作らず同一Attempt内で継続するため、`sourceType`が復習ラウンド中に元の値から変化しないことも確認済み。`features/history/quiz-start-integration.js`の`startAttemptForQuiz()`へパラメータを追加して`createAttempt()`へ橋渡しし、`features/history/learning-record-sync-integration.js`の`syncStartAttempt()`から`startAttempt` payloadへ`sourceType`/`testSetId`を追加送信する配線とした。実本番学習記録GASへ4種類の`sourceType`（`normal`/`weak_review`/`dormant_review`/`testset`）を実際に送信し、`getStudentHistory`で保存値を実測（`testset`のみ`testSetId`が実IDで保存され、他3種は空文字列として正しく正規化されることを確認）。ブラウザreload E2E自体は今回実施していないが、実GAS保存値を`createAttempt(gasAttempt)`へ渡すことで`sourceType`/`testSetId`が既存の`startedAt`と同じ仕組みで維持されることを確認した（reload/new context跨ぎの永続復元自体はPhase5-5で本番経路のまま確認済み）。旧データ（`sourceType`/`testSetId`が未設定、またはGAS側で空文字列）でも`createAttempt()`がクラッシュせず正常復元できることを確認済み。headless Chromeによるブラウザテスト（Case1〜17、モックGAS）26/26 OK。GAS（`Code.gs`/`SheetHelpers.gs`）・Spreadsheet schemaへの変更は0件（Phase5-2で列・validationとも先行実装済みのため）。`services/learning-record-service.js`・`features/history/learning-record-restore-integration.js`も無変更。テストで本番Spreadsheetへ書き込んだ架空データ（`TEST_PHASE5_6_STUDENT_GAS`）はcleanup済み（attempts/answerRecordsとも0件を実機確認）。**Phase5-7（Weakness暫定閾値の妥当性検証）・Phase5-8（想起ファーストUXのchoice先行実装）・Phase5-9（想起ファーストUXの他mode対応＋画像付加要素拡張）はまだ未着手。**

**運用判断（2026-08-23（日）確定）**：Phase5-0〜5-6は完了、Phase5-7〜5-9は未完了のまま、問題マスター拡充（Phase6）を先行して開始する。理由：(1)Phase5-7は実運用データが無ければ本質的に検証できないTaskであり、実生徒の学習データが一定量蓄積してから着手する、(2)Phase5-8/5-9は学習効果向上のUX改善であり、現状の`choice`等でも通常利用可能なため問題作成・教室運用開始のブロッカーにしない、(3)Phase5-9（画像付加拡張）を後回しにしても、`imagePath`列が理科CSVに既存かつ`question-normalizer`がmode非依存で正規化済みのため、既存CSV schemaへの大規模な手戻りは生じない。以上より、来週の教室運用開始へ向け当面は問題データ作成を優先する。詳細な判断根拠・比較検討は本方針確定に至った調査記録を参照（`docs/architecture/ls-total-test-system-design-v1.md` Phase6節「問題CSVへの影響」参照）。**Phase5全体の完了とはしない**（Phase5-7〜5-9は運用データ・実施状況を見ながら後日改めて着手する）。
