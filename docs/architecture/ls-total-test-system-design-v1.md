...
```
features/
├── ranking/            … ランキング取得・表示ロジック（Phase6）
├── weak-questions/      … 苦手問題判定ロジック（Phase5）
├── timer/               … タイマー・ペナルティ計算（Phase7）
├── test-range/          … 学校別テスト範囲の解決ロジック（Phase4）
...
```
学校（schoolId） → 学年（gradeId） → 学年度（academicYearId） → 定期テスト回（examRoundLabel） → 教科（fieldId） → 対象単元 or 問題セット（questionSetId）
```

`TestRange`エンティティの属性は `docs/specification/domain-model-v1.md` を参照。

### 9.2 学校名の表記揺れ対策（確定）

学校名の文字列（例:「〇〇中学校」「〇〇中」の表記ゆれ）をテスト範囲マスタのキーに使わず、**生徒管理システムが発行する`schoolId`**を使う。これにより`TestRange`マスタと生徒管理マスタは`schoolId`で接続する。

### 9.3 生徒管理マスタとの接続方法（確定方針・詳細は要確認）

1. 生徒選択・ログイン時に、学習アプリは生徒管理システム（またはそれと同一の可能性があるGAS、1.2節参照）から `studentId → schoolId, gradeId` を取得する。
2. 取得した`schoolId`・`gradeId`をキーに、学習アプリ用`TestRange`マスタ（Google Sheets新設）を検索し、対象単元・問題セットを絞り込みフィルタへ反映する。

**要確認**: 生徒管理システムのレスポンスに`schoolId`が既に含まれているか（現行`normalizeStudentRecord`は`grade`のみを持ち`school`は含まれていない）。含まれていない場合はAPI拡張が必要（7章の「生徒プロフィール取得」）。

### 9.4 未設定時のフォールバック（仮決定）

該当`schoolId`のテスト範囲が`TestRange`マスタに存在しない場合（新規校・未登録校）は、「学校別範囲なし」として全単元を表示するフォールバックとする。これにより、テスト範囲マスタの整備が追いつかない学校の生徒であっても、機能自体は使い続けられる。
...
| 3（Phase6以降） | ranking計算・timer/penalty計算テスト | 同上の`node:test`を継続利用 |
...
| 項目 | 内容 |
|---|---|
| 目的 | `TestRange`マスタ導入、学校別出題範囲の絞り込み実現 |
| 前提条件 | Phase2完了（生徒プロフィールAPIで`schoolId`/`gradeId`が取得できること） |
| 主な変更対象 | `features/test-range/`新設、`getTestRange`API、`TestRange`用Sheets新設 |
| 完了条件 | 最低1校のテスト範囲設定で、対象単元への絞り込みが正しく反映されること |
| 回帰確認 | テスト範囲未設定校でも全単元表示にフォールバックすること |
| ロールバック方法 | `TestRange`マスタが空でも動作するフォールバック設計のため、機能フラグ的に無効化可能 |
| 次Phaseへ進む判定 | 実データでの絞り込み確認＋人間レビュー承認 |

### Phase5: 学習分析（苦手問題・おすすめ問題・履歴）

| 項目 | 内容 |
|---|---|
| 目的 | 10章のルールベース苦手問題判定、学習履歴画面の実装 |
| 前提条件 | Phase2完了（AnswerRecordが十分蓄積されていること） |
| 主な変更対象 | `features/weak-questions/`新設、学習履歴・苦手問題画面新設 |
| 完了条件 | 実データで苦手問題が妥当な範囲で抽出されること（暫定閾値の妥当性を人間が確認） |
| 回帰確認 | 既存の間違い直し機能（直近の不正解のみ再出題）が引き続き動作すること |
| ロールバック方法 | 新規画面・新規モジュールの削除のみで切り戻し可能 |
| 次Phaseへ進む判定 | 暫定閾値の妥当性確認＋人間レビュー承認 |

### Phase6: ランキング

| 項目 | 内容 |
|---|---|
| 目的 | 11章・ranking-spec-v1.mdの仕様どおりランキング機能を実装 |
| 前提条件 | Phase2完了（Attempt完了判定が正しく機能していること） |
| 主な変更対象 | `features/ranking/`新設、`RankingRecord`用Sheets新設、`getAnnualRanking`/`getAllTimeRanking`/`upsertBestRecord`API |
| 完了条件 | 完了判定・ベスト記録更新・同点処理が仕様どおり動作すること |
| 回帰確認 | 途中終了したAttemptがランキングに反映されないこと |
| ロールバック方法 | ランキング画面・APIの無効化のみで切り戻し可能（既存Attempt/AnswerRecordは無関係） |
| 次Phaseへ進む判定 | 複数生徒での実データ確認＋人間レビュー承認 |

### Phase7: 時間制限・タイムアタック

| 項目 | 内容 |
|---|---|
| 目的 | 12章のタイマー・ペナルティを実装 |
| 前提条件 | Phase6完了（ペナルティ込みタイムをランキングに反映するため） |
| 主な変更対象 | `features/timer/`新設、出題画面へのタイマー表示追加 |
| 完了条件 | 時間切れ時の挙動（12.2の推奨案どおり）が仕様どおり動作すること |
| 回帰確認 | タイマー無効設定の問題セットでは従来どおり無制限で動作すること |
| ロールバック方法 | タイマー設定を全問null（無制限）に戻せば実質無効化できる |
| 次Phaseへ進む判定 | 実機での時間切れ挙動確認＋人間レビュー承認 |

### Phase8: 画像・地図・並び替え強化

| 項目 | 内容 |
|---|---|
| 目的 | 理科の画像問題・実験手順並び替え・社会の年表問題・地図問題拡張 |
| 前提条件 | Phase3完了（理科分野の基盤があること） |
| 主な変更対象 | `QUESTION_MODE_HANDLERS`への新形式追加（画像問題）、既存`sort-renderer.js`の理科実験手順への流用、`MAP_CONFIGS`への地図追加 |
| 完了条件 | 理科の実験手順並び替えが既存`sort-renderer.js`の流用で（重複実装せず）動作すること |
| 回帰確認 | 既存の歴史の並び替え問題が引き続き動作すること |
| ロールバック方法 | 新規モードハンドラの削除のみで切り戻し可能 |
| 次Phaseへ進む判定 | 理科・社会それぞれ最低1問での動作確認＋人間レビュー承認 |
