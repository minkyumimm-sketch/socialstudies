...
| 2026-08-16 | Phase4本番E2E（Task56）に伴うコード調査で判明した事項を11章として追記（Home統計の再計算タイミング、Attempt/AnswerRecordの永続化方式）。Phase0時点の1〜10章の内容は変更していない |
...
## 11. Phase4以降で判明した追加事項（2026-08-16追記）

**本章はPhase0完了後、Phase4本番E2E（Task56、2026-08-16実施）に伴うコード調査で判明した事項を追記したものです。Phase0時点の調査内容（1〜10章）は変更していません。コード変更は行っていません。**

### 11.1 既知事項A：Home統計の再計算タイミング

`renderHomeForStudent()`（`features/home/home-renderer.js`）は生徒選択時（`app.js`の生徒名入力・選択ハンドラ）にのみ実行される設計であり、Quiz/TestSet終了後に単純にHome画面へ戻っただけでは呼び出されない。そのため、通常学習・TestSetのどちらで学習した場合でも、学習後にHomeへ戻っただけでは苦手問題数等の表示が古いままになる場合がある。生徒を選び直すと再計算される。

- **TestSet固有の不具合ではない**（通常学習でも同様の挙動）。
- Phase4ではコード修正しない。
- 将来のHome/History/Weakness改善、特にPhase5「学習分析」実装時の改善候補として記録する。

### 11.2 既知事項B：Attempt / AnswerRecordの永続化方式

Phase4本番E2Eに伴うコード調査で、`features/history/`・`features/repository/`配下のAttempt/AnswerRecordドメイン層が`features/storage/memory-storage.js`（`createMemoryStorage()`、ブラウザメモリ内Map）をデフォルトの保存先として使用しており、GAS永続化用に用意された`features/storage/gas-storage.js`は全メソッドが`throw new Error("Not implemented")`の未実装スタブであることが確認された。

一方、既存の生徒管理・解答保存用GAS（`services/gas-service.js`の`GAS_WEB_APP_URL`、`services/student-service.js`の`saveAnswerRecord`）はこれとは別系統として引き続き稼働しており、変更されていない。

- この新しいドメイン層（History/Weakness等が参照するAttempt/AnswerRecord）のデータは、**ブラウザリロードをまたいだ永続性が保証されていない**。
- **Phase2で意図的にスコープ外とされた既存事項であり、Phase4で新たに発生した不具合ではない**（`features/storage/gas-storage.js`のコード内コメントに「本タスクのスコープ外」と明記されている）。
- 将来、学習分析・履歴機能を本格運用する前に、永続化方式（GAS接続の実装、または別方式）を確定する必要がある技術的負債として記録する。本レポートではGAS接続の実装は行わない。
