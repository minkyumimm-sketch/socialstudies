...
| 5 | `sourceType` | string | 必須 | `normal`/`weak_review`/`dormant_review`/`testset`/`testset_review`のいずれか（既存`SOURCE_TYPE_VALUES`をそのまま再利用。`testset_review`はPhase3D-4A前提で追加、2026-09-13時点で本番GASが5値を受理することを実測確認済み）。 |
...
| 15 | `runId` | string | `sourceType="testset"/"testset_review"`のみ必須 | それ以外は**指定禁止**。Phase4E-0A前提で追加。**2026-09-13時点で本番Spreadsheet/GASへの反映を実測確認済み**。詳細は`docs/operations/learning-record-gas/RunIdentity.gs`。 |
| 16 | `reviewRound` | 整数 | `sourceType="testset"/"testset_review"`のみ必須 | `testset`は0固定、`testset_review`は1以上。それ以外は**指定禁止**。Phase4E-0A前提で追加。**2026-09-13時点で本番Spreadsheet/GASへの反映を実測確認済み**。 |
