...
| `runId`/`reviewRound`のルール（Phase4E-0A/0B前提） | `sourceType="testset"`は`runId`必須・`reviewRound=0`固定。`sourceType="testset_review"`は`runId`必須・`reviewRound`は1以上の整数。それ以外のsourceTypeはいずれも指定禁止（domain-model-v1.md 3.11.4節、`docs/operations/learning-record-gas/RunIdentity.gs`参照）。**Phase4E-0B移行モード**: 上記はnew payload（新Web）の契約。`testset`/`testset_review`で`runId`/`reviewRound`を両方省略したlegacy payload（旧Web）は、移行期間限定で正常系として許容し`runId=""`/`reviewRound=""`で保存する。片方だけの指定は常にエラー |
