...
| 任意項目 | `sourceType`（`normal`/`weak_review`/`dormant_review`/`testset`/`testset_review`/`memorize`、未送信時はサーバ側で起点不明として扱う。`testset_review`はPhase3D-4A前提で追加、domain-model-v1.md 3.11.3節参照。`memorize`は暗記モード-0前提で追加、3.11.6節参照。sourceTypeの許可値そのものはGAS側`SOURCE_TYPE_VALUES`が正本、`docs/operations/learning-record-gas/MemorizeSourceType.gs`参照）, `testSetId`（`sourceType="testset"`または`sourceType="testset_review"`のときのみ値を持つ、それ以外（`memorize`含む）は`null`）, `runId`/`reviewRound`（`sourceType="testset"`/`"testset_review"`/`"memorize"`のときのみ必須、それ以外は指定禁止。domain-model-v1.md 3.11.4節・3.11.6節参照。本番反映手順は`docs/operations/learning-record-gas/RunIdentity.gs`）。**Phase4E-0B前提**: GAS側は無停止移行のため、`testset`/`testset_review`に限り移行期間限定で「new payload」（`runId`/`reviewRound`を両方指定、新Web契約）と「legacy payload」（両方省略、旧Web互換）の両方を受け付ける（`ALLOW_LEGACY_RUN_IDENTITY_PAYLOAD_`フラグ、片方のみの指定は常にエラー）。Web側は常にnew payloadのみを送信し、legacy payload送信ロジックは持たない。**暗記モード-0前提**: `sourceType="memorize"`にはこのlegacy payload許容は適用されない（既存の本番トラフィックが無い新規sourceTypeのため、`runId`/`reviewRound`は常に両方必須）。暗記モード自体のUI/runner/resumeは今回未実装のため、実際に`sourceType="memorize"`が送信されることはまだ無い |
| エラー | 必須項目欠落 |
...
| `sourceType`/`testSetId`のルール | 既存`handleStartAttempt`と同じルールを踏襲：`sourceType="testset"`または`sourceType="testset_review"`のときのみ`testSetId`必須、それ以外（`memorize`含む）では`testSetId`の指定自体を禁止（エラー）。`sourceType`は`attempt_progress`では省略不可（既存`Attempt.sourceType`は後方互換のため省略可だが、`attempt_progress`はレガシーデータを持たない新設テーブルのため、再開候補の判定を単純化する目的で必須とする、Phase3B-1確定）。`testset_review`はPhase3D-4A前提で追加（domain-model-v1.md 3.11.3節）、Phase3D-4A時点ではこのsourceTypeを実際に送信するWeb側の経路はまだ存在しない。`memorize`は暗記モード-0前提で追加（domain-model-v1.md 3.11.6節）、暗記モード-0時点でもこのsourceTypeを実際に送信するWeb側の経路はまだ存在しない（UI/runner未実装） |
| `runId`/`reviewRound`のルール（Phase4E-0A/0B・暗記モード-0前提） | `sourceType="testset"`は`runId`必須・`reviewRound=0`固定。`sourceType="testset_review"`/`"memorize"`（暗記モード-0で追加）は`runId`必須・`reviewRound`は1以上の整数。それ以外のsourceTypeはいずれも指定禁止（domain-model-v1.md 3.11.4節・3.11.6節、`docs/operations/learning-record-gas/RunIdentity.gs`参照）。**Phase4E-0B移行モード**: 上記はnew payload（新Web）の契約。`testset`/`testset_review`で`runId`/`reviewRound`を両方省略したlegacy payload（旧Web）は、移行期間限定で正常系として許容し`runId=""`/`reviewRound=""`で保存する。片方だけの指定は常にエラー。**`memorize`にはこのlegacy許容は適用されない**（既存の本番トラフィックが無い新規sourceTypeのため、常に両方必須） |
...
| エラー | 必須項目欠落、`questionIds`/`wrongQuestionIds`が不正JSON・非配列・重複、`currentQuestionIndex`/`retryRound`が負数、`currentQuestionIndex`が対象配列長を超える（配列長と同値は有効）、`status`/`sourceType`が許可値以外、`sourceType`と`testSetId`の組み合わせ不正、`sourceType`と`runId`/`reviewRound`の組み合わせ不正（Phase4E-0A前提）、`retryWrongEnabled`がboolean以外、新規行作成時に該当`attemptId`が`attempts`シートに存在しない・`studentId`不一致、既存行更新時に既存`attempt_progress`行と`studentId`不一致 |
